...
def likelihood(travel_time, t_a, mu_b, mu_g, mu_t, sigma, sigma_t):
    ...
    cdf_b = lambda b: norm.cdf(b, mu_b, sigma)
    cdf_g = lambda g: norm.cdf(g, mu_g, sigma)
    pdf_b = lambda b: norm.pdf(b, mu_b, sigma)
    pdf_g = lambda g: norm.pdf(g, mu_g, sigma)
    # cdf_b = lambda b: truncnorm.cdf(b, -mu_b / sigma, (1 - mu_b) / sigma, loc=mu_b, scale=sigma)
    # cdf_g = lambda g: truncnorm.cdf(g, (1 -mu_g) / sigma, 100000, loc=mu_g, scale=sigma)
    # pdf_b = lambda b: truncnorm.pdf(b, -mu_b / sigma, (1 - mu_b) / sigma, mu_b, sigma)
    # pdf_g = lambda g: truncnorm.pdf(g, (1 - mu_g) / sigma, 10000, mu_g, sigma)
    ...
    likelihood_kink = norm.pdf(t_a, mu_t, sigma_t) * (1 - cdf_b(b0)) * (1 - cdf_g(g0))
    ...
    def inner_int_b(b):
        ...
        return (norm.cdf(bs[1], mu_t, sigma_t) - norm.cdf(bs[0], mu_t, sigma_t)) * pdf_b(b)
    
    def inner_int_g(g):
        gs = find_gs(g, travel_time)
        return (norm.cdf(gs[1], mu_t, sigma_t) - norm.cdf(gs[0], mu_t, sigma_t)) * pdf_g(g)
