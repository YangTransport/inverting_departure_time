...
Let thus \(I\) be a random variable which describes the distribution of internal minima,
\(K\) a random variable which describes the distribution of kink minima.

The pdf can be decomposed in

\begin{equation}
  \label{eq:likelihood_split}
  f_{T_a}(t_a) = \frac{f_I(t_a) + f_K(t_a)}{2}
\end{equation}
...
\begin{align*}
  ...
  & = \mathbb{P}(t^* = t_a)\int_{\beta_0(t_a)}^1\mathbb{P}(\beta = b)\, db\int_{\gamma_0(t_a)}^\infty\mathbb{P}(\gamma = g)\, dg \\
\end{align*}
